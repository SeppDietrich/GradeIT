...
<div class="home-page">
  <div class="hero">
    <h1>📊 GradeIt</h1>
    <p class="subtitle">Creează sisteme de grading pentru orice dorești.<br>Evaluează, compară, decide.</p>
  </div>
  ...
  <div class="auth-card">
    <div *ngIf="!showEmail">
      ...
      <button class="btn btn-outline" (click)="showEmail = true">Continuă cu Email</button>
    </div>
    ...
    <div *ngIf="showEmail">
      ...
      <div class="btn-group">
        <button class="btn btn-primary" (click)="loginEmail()">Autentifică-te</button>
        <button class="btn btn-outline" (click)="registerEmail()">Cont nou</button>
      </div>
      <button class="btn-link" (click)="showEmail = false">← Înapoi</button>
    </div>
  </div>
</div>
  `,
  styles: [`
.home-page { display: flex; flex-direction: column; align-items: center; justify-content: center; min-height: 80vh; padding: 2rem; }
.hero { text-align: center; margin-bottom: 2rem; }
.hero h1 { font-size: 2.5rem; font-weight: 500; margin-bottom: 0.5rem; }
.subtitle { color: var(--color-text-secondary); line-height: 1.6; }
...
.field input { width: 100%; padding: 8px 10px; border: 0.5px solid var(--color-border-secondary); border-radius: var(--border-radius-md); font-size: 14px; background: var(--color-background-primary); color: var(--color-text-primary); }
...
export class HomeComponent {
  ...
  async loginEmail() {
    try {
      ...
      this.errorMsg = 'Email sau parolă incorectă.';
    }
  }
